...
class Valida{
	...
	function ENumerico(){
		if(!is_numeric($this->valorcampo)){
			$this->erro.="O valor ".$this->nomecampo." não á numérico. Informe apenas números e ponto.<br>\n";
		}
	}
	...
	function FormatoEmail(){
		...
		if((!eregi("@", $this->valorcampo) || !eregi("\.", $this->valorcampo)) && (strlen($this->valorcampo)>0)){
			$this->erro.="O campo ".$this->nomecampo." possui um formato inv�lido.<br>\n";
		}
	}
	...
	function checacpf(){
		$cpf = ereg_replace("[^0-9]", "", $this->valorcampo); 
		if (strlen($cpf)!=11) 
		...
		{ 
		  ...
		  if(in_array($cpf, $nulos)) $this->erro.="O campo ".$this->nomecampo." possui um formato inv�lido.<br>\n";
		  else 
    		  { 
	    	   $acum=0; 
	    	   ...
	    	   if($compdv <> $dv) $this->erro.="O campo ".$this->nomecampo." possui um formato inv�lido.<br>\n";; 
	  		} //fim else
		} //fim else
	}
	function PegaErros(){

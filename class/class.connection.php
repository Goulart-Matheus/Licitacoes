...
class DataBase
	...
	function __construct($host, $port, $database, $user, $pass)
	{
		$this->host			=$host;
		$this->port			=$port;
    ...
    function listColumn($tabela){
        ...
        while($i--)
            $array_out[]=pg_field_name($result,$i);
        return $array_out;
    }
}
...
class Query {
	...
	function __construct($bdInstance) {
		$this->bd			=$bdInstance;
	}
    ...
    function lastInsert() {
        ...
        if(pg_num_rows($result)) $this->last_insert =pg_fetch_array($result, 0, PGSQL_NUM);
	...
	function exec($sql)
	{
		...
		global $debug, $debug_info;
  		...
  		if($sql=="") {
			...
			$this->result(-1);
			if(pg_last_error($this->bd->connection)) {
                $this->sql_err=1;
                $this->sql_err_msg=pg_last_error($this->bd->connection);
                $this->abort();
                return 0;
            }
        	return $this->consulta;
